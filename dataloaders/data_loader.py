...
class PRELUDEDataset:
    ...
    def _load_lp_splits(self):
        """Loads all pre-split link files generated by create_splits.py."""
        print("Loading pre-split link prediction data...")
        split_names = ['train_lp', 'valid_inductive', 'test_inductive', 'test_transductive']
        for split in split_names:
            path = os.path.join(self.data_dir, f"{split}_links.dat")
            if os.path.exists(path):
                with open(path, 'r') as f:
                    for line in f:
                        ...
                        if len(parts) >= 4:
                            self.links[split].append(
                                (int(parts[0]), int(parts[1]), int(parts[3])) # (src_gid, tgt_gid, label)
                            )
    ...
    def summary(self):
        ...
        print(f"  - Training Set:       {len(self.links['train_lp'])} links")
        print(f"  - Validation Set:     {len(self.links['valid_inductive'])} links")
        print(f"  - Test Set 1 (Trans): {len(self.links['test_transductive'])} links")
        print(f"  - Test Set 2 (Induc): {len(self.links['test_inductive'])} links")
        print("---------------------------------")
